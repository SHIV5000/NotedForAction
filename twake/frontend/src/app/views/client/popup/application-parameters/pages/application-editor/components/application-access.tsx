...
export const ApplicationAccess = ({
  ...
  return (
    <>
      <Row>
        <Title level={3}>{Languages.t('Noted For Action.application.access.title')}</Title>
      </Row>
        ...
        return (
          <div key={key === 'hooks' ? 'listened events' : key} className="bottom-margin">
            ...
            {defaultApplicationScopes ? (
              ...
              <Text type="secondary">
                {Languages.t('Noted For Action.application.access.no_default_scopes_available', [key])}
              </Text>
            )}
          </div>
        );
