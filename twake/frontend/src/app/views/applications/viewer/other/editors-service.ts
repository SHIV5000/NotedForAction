...
export const useEditors = (
  ...
  const apps = applications.filter(
    app =>
      app.display?.Noted For Action?.files?.editor?.preview_url ||
      app.display?.Noted For Action?.files?.editor?.edition_url,
  );
  ...
  apps.forEach(app => {
    if (
      (app.display?.Noted For Action?.files?.editor?.extensions || []).indexOf(
        ((extension || '') + (options?.url ? '.url' : '')).toLocaleLowerCase(),
      ) >= 0
    ) {
      if (app.display?.Noted For Action?.files?.editor?.edition_url) {
        editor_candidate.push({ app });
      }
      if (app.display?.Noted For Action?.files?.editor?.preview_url) {
        preview_candidate.push({
          url: app.display?.Noted For Action?.files?.editor?.preview_url,
          app: app,
        });
      }
    }
  ...
  const openFile = (app: any, fileId: string, driveId: string) => {
    ...
    window.open(getFileUrl(app.display?.Noted For Action?.files?.editor?.edition_url, fileId, driveId));
  };
